...
| **REAL-DATA-07** | [Replication and the illumination envelope on 42 real pairs](REAL-DATA-07_replication_and_envelope.md) | Replicate the low-incidence success with an independent frame, and measure each engine's success rate against Δincidence over every archived frame on two ground windows, with criteria frozen before any new tile was fetched | **COMPLETE — S4, S2 MET; S1, S3, S5, S6 NOT MET** | 42 geometry-confirmed pairs, 14 frames, two engines, **0 wrong passes in 37**; pooled Δincidence separation **p = 0.0042** (RD-03 0.0040, RD-04 0.121); no success above 40°; B4L's yield 5–25× B1's inside the envelope but the same envelope (17 / 42 vs 20 / 42). **Replication NOT MET:** E1 excluded (target outside swath), E2 fails vs A (6, INCONSISTENT) and vs B (3). North-up moved A → B (RD-04) from 7 to 9 inliers across the rule (S6) | **Four frames fail against every partner irrespective of Δincidence** (E2 at 21°; the three darkest tiles at 66.9–74.7°, one clamped to the frame edge) — the frame-identity question of D-040-N1 returns with n = 4. **E-036**: the raw reproduction arm first ran in the wrong direction; caught by its own gate | D-049 (scope re-stated), D-047-N1 (native-scale envelope claim withdrawn); a 60–75° incidence sweep; E2 investigated before any further replication |
