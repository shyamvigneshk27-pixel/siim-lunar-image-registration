...
| **REAL-DATA-04** | [Can frame identity and illumination be separated?](REAL-DATA-04_illumination_vs_frame_identity.md) | Break the Δincidence / frame-identity confound with one more real low-incidence frame, against a decision table frozen before the data existed | **COMPLETE — ANSWERED. D↔A succeeds, D↔B fails** | **Overlap CONFIRMED first: 97.87 % / 71.30 % / 70.35 %**, with the two decisive edges matched to **0.95 pp**. The unmodified baseline then gives **1656 inliers** (ratio 0.9414, coverage occupancy **1.000**) at Δincidence **11.73°** and **3** at **51.54°** — **D↔A succeeds, D↔B fails: the first row of the pre-registered table.** Across **six** real edges, five frames and two ground windows, **every frame now appears in both a succeeding and a failing edge**, so frame identity predicts nothing **across that set** and **Δincidence predicts all six**. The succeeding edge's scale matches SPICE-derived `SCALED_PIXEL` to **0.04 %** and is the only edge in either stage below the corner-polygon discrimination floor (0.53×). **Illumination attributed (D-040); frame identity substantially weakened, NOT conclusively refuted — A and D each have n = 1 in the successful regime (see D-040's superseding note and REAL-DATA-05).** | **E-032**: the `LRO_FLIGHT_DIRECTION` rule for which latitude line 0 sits at is **not a rule** — 8/10, two counterexamples; the `NORTH_AZIMUTH` cross-check quoted for it had **no discriminating power** in the sample it was validated on, and tracks the *cross-track* sense instead. Prose corrected, no code defect | **REAL-DATA-05** — replicate the result on a second low-incidence frame |
| **REAL-DATA-05** | [Does the illumination result replicate on a second low-incidence frame?](REAL-DATA-05_illumination_replication.md) | Replicate REAL-DATA-04 with an independent low-incidence frame E over the same ground, using the unchanged matcher, against criteria frozen before any screening | **CLOSED — UNRESOLVED, BY DATA AVAILABILITY** | The pre-registered screen ran as written and returned **zero** admissible frames at **every** tier and rung (T1/T2 × S1/S2, 261 and 906 products). Of 906 CDR products, exactly **8** can centre a full tile on ground shared with A, B and D; only **2** are in the required incidence band, and **both are acquired in the opposite along-track sense**, rejected by H5 — which is REAL-DATA-04's own orientation filter (D-038), unchanged. The two filters this stage added rejected **zero** candidates. **No image byte was fetched, no registration was run, and the decision table was never reached.** D-040 is neither upheld nor reduced; its replication debt stands | **F-1**: the stage stopped rather than relax the band by 0.22° to admit a frame, relax H5, drop D from the intersection, or move the target silently. All four empty screens are preserved | **EXP-004** is now the *measured* blocker on the replication, not merely an open question |
| **EXP-006** | Protocol vs matcher on lunar data *(no stage report exists)* | Test the project's core architectural hypothesis — *the matcher is a replaceable part; the protocol is the contribution* — on lunar imagery. The 33× protocol effect it rests on is from **SAR-optical** imagery and a **single preprint** (`../sources.md` S4), carried with an explicit transfer caveat | **NOT STARTED and NOT PRE-REGISTERED.** Unlike EXP-004 there is no frozen Part 1, no hypotheses, no success criterion and no artefact directory. It is referenced in the README and in four design documents as the experiment that would settle the thesis; **it is a named intention, not a registered stage** | *(none — the stage has not run)* | *(none)* | — |
| **EXP-010** | [Sub-pixel refinement](EXP-010_subpixel_refinement.md) | Define sub-pixel accuracy against exact ground truth, measure it on real NAC self-warps, and measure how Sun-azimuth change biases local refinement | **COMPLETE — gate, S1, S2, S3 MET; S4 undefined** | Local ECC refinement reaches **0.003 px** median (95 % CI 0.0031–0.0033) on real self-warps of all four recorded tiles (an upper bound); on synthetic terrain the refined error grows from 0.04 → 0.23 px (mare) and 0.008 → 0.07 px (highlands) between Δaz 0° and 30° and stays under 0.25 px throughout, so the sub-pixel definition holds to the largest Δaz tested; phase correlation recovers only ~half of a residual's magnitude and is not deployed; no point was ever made worse, so H4 is untestable here | **E-034**: the default **affine** model recovers a pure integer self-shift of frame D to **0.99 px median / 2.31 px max** with 11 849 inliers and a zero fit residual (translation model: 0.002 px) — correlated localisation error absorbed into the linear terms; refinement removes it (0.30 → 0.009 px) | Model selection by held-out evidence (pre-registration pending); no real cross-illumination sub-pixel claim until check points exist |
| **EXP-011** | [Transform model selection](EXP-011_model_selection.md) | Decide the transform model by evidence, and test whether re-estimation from refined points removes the E-034 bias | **COMPLETE — gate, S2, S3 MET; S1 NOT MET** | Refine-then-reselect (rule B) picks the true model in **48/48** self-warp cases and reaches **0.0018 px** dense error (CI 0.0013–0.0033) against **0.0975 px** for the affine default; held-out residuals of translation and affine on frame D are indistinguishable (0.0338 vs 0.0336 px), so fit-based selection works here only through its simplicity tie-break | H1's predicted wrong selection did **not** occur (S1 NOT MET): the mechanism held, the consequence did not — recorded as a partial refutation | Pipeline order becomes estimate → refine → re-estimate; verdict gains `model_selected_by` |
| **REAL-DATA-06** | [Photometric normalisation](REAL-DATA-06_photometric_normalisation.md) | Does normalising both tiles to a standard illumination geometry (Lommel-Seeliger, Hapke-HG) before matching change the outcome on the four real edges that fail? | **CLOSED — S4, S3 MET; S1, S2 NOT MET, BY CONSTRUCTION (E-035)** | Both frozen arms return counts and matrices **identical** to the uncorrected run on all six edges (4, 5365, 4, 7, 3, 1656): a per-frame correction is a per-frame scalar, and the pipeline's per-image stretch removes it. An exploratory per-pixel arm from the 59 m SLDEM changes counts (5365 → 2469, 1656 → 1197) and converts **no** failing edge at any of five rungs | **E-035**: the pre-registered design could never have changed any outcome; the identity check that would have shown it (corrected-stretched ≠ raw-stretched) was not part of Part 1 | Per-pixel correction only at a fine-DEM site (SERENRIDGE1); D-048 |
| **EXP-007** | [DEM-conditioned correspondence and the learned engine](EXP-007_dem_conditioned_correspondence.md) | If each image is matched against a DEM rendered under its own Sun, do the failing real edges register, and at which GSD does the 59 m DEM stop being enough? Plus a licensable learned engine (DISK + LightGlue) as the zero-shot arm | **COMPLETE — S2, S3, S4, S5 MET; S1 NOT MET; S6 NOT MET for the two DEM-render arms** | The SLDEM render carries **no matchable structure** on this mare window at 1.8, 3.6, 7, 15 or 30 m (0–19 render keypoints on high-Sun frames; every leg 0–7 inliers; 0 of 4 failing pairs at any rung). **DISK + LightGlue registers C → A at 38.85° with 56 geometry-consistent inliers** where RootSIFT gives 4, and at 7–30 m registers three of the four ~40° pairs with 447–2042 consistent inliers; it does not cross 51.54°. RootSIFT itself passes two ~40° pairs marginally (9–10 inliers) at 3.6–15 m | H0 not supported at any tested rung with this DEM (**D-046**, demoted to conditional); the two render arms also fail both succeeding pairs (S6). E-035 surfaced here | B4L admitted as engine arm (**D-047**, supersedes D-028); envelope on 42 pairs in REAL-DATA-07; radar and 100 m proxies in REAL-DATA-08; fine-DEM site next |
| **REAL-DATA-07** | [Replication and the illumination envelope on 42 real pairs](REAL-DATA-07_replication_and_envelope.md) | Replicate the low-incidence success with an independent frame, and measure each engine's success rate against Δincidence over every archived frame on two ground windows, with criteria frozen before any new tile was fetched | **COMPLETE, AMENDED SAME DAY (E-037) — amended run: S1, S2, S4, S5 MET; S3, S6 NOT MET** | Original run (quarter-turn orientation): p = 0.0042, replication NOT MET, four frames failing everyone. Amended run (mirrored tiles flipped, three engines): **replication MET** (E2 → A 2138 consistent, E2 → B 4), pooled **p = 0.0012** (RD-03 0.0043, RD-04 0.029), last ≥ 0.8 bin 20–25°, nothing above 40°, **0 wrong passes in 76**, successes 25 / 24 / 27 of 42 (B1 / B4L / B4X); three-engine agreement ≤ 2.16 px vs ≥ 247 px. North-up still moves A → B (RD-04) 7 → 9 (S6) | **E-037**: five of fourteen census frames are mirror images by their own corner metadata; the quarter-turn orientation could not undo it and two of the "four frames that fail against everyone" were exactly those. The other two are the darkest tiles at 72–75° (ceiling, RL-042c). **E-036**: the raw reproduction arm first ran in the wrong direction; caught by its own gate | D-040-N2 (replication discharged), D-049-N2 (re-scoping withdrawn), D-047-N1 (native-scale envelope claim withdrawn), D-051 (agreement floor 3 px, B4X admitted); a 70–75° incidence sweep |
| **REAL-DATA-08** | [Cross-sensor and cross-modality proxies: NAC ↔ Mini-RF radar (15 m), NAC ↔ WAC (100 m)](REAL-DATA-08_multimodal_proxies.md) | Measure the pipeline on the two open archive products over the recorded ground that change the physics most — a radar image (modality change) and a 100 m mono mosaic (the IIRS/WAC rung) — labelled PROXY throughout | **COMPLETE — S1 MET; S2, S3 NOT MET** | **Radar registers under no engine** (B1, B7, B4L each 0 / 16; best attempt 8 inliers, 193 px wrong). **100 m rung:** B1 1 / 4 frames (D, 9 inliers, consistent); **B4L 34–99 geometry-consistent inliers on three of four frames** from 111 × 46 px NAC strips, ≈ 2 px against a ≈ 2.3 px floor, plus one wrong pass (B at 59 m). 126 rows, 5.8 min | The multimodal claim is a measured negative on a real radar proxy; the learned engine's first wrong pass (1 / 23 across RD-07/08); the runner used a box average where Part 1 named a PSF-aware operator (deviation recorded, R9 implemented afterwards) | D-050; a cross-modality engine (MatchAnything-ELoFTR, licence check first) is the named next arm; nothing here is a Chandrayaan-2 result |
| **REAL-DATA-09** | [Chandrayaan-2 ingestion and the first Chandrayaan-2 ↔ LRO NAC registrations](REAL-DATA-09_chandrayaan2_ingestion.md) | Ingest OHRC, TMC-2 (L1 and L2 GeoTIFF ortho + DEM) and IIRS (cube + geometry file) through a verified contract, then run the TMC-2, OHRC, IIRS and 320:1 rungs against the recorded NAC ground, with the TMC-2 DEM as the fine-DEM test of H0 | **PRE-REGISTERED 2026-09-05, NOT STARTED** — Part 1 frozen before any Chandrayaan-2 byte is on disk; waits on the user's PRADAN download (2026-09-06) | *(none — the stage has not run)* | *(none)* | Part 2 once the products exist; the acknowledgement wording (S15) enters the deliverable's licence section |
...
tests). After **REAL-DATA-02**: **357 passed, 2 skipped** (260 → 357; +71 footprint geometry, +21 index table, +5 PDS4 display direction). After **REAL-DATA-04**: **398 passed, 2 skipped** (376 → 398; +12 frame-D screening including the orientation signature of E-032, +10 tile-admissible geometry). After **REAL-DATA-03**: **376 passed, 2 skipped** (357 → 376; +6 raw-tile cache, +13 tile-window and predicted-correspondence geometry). After **REAL-DATA-05** and the demo track: **497 passed, 2 skipped** (+20 frame-E screening, +32 demo real-data integration, +2 verdict integrity tests recovered from an uncommitted git index blob, +10 pinning the pre-registered decision constants across all four sites where they are declared, +24 demo evidence-integrity and provenance regressions).
...
**Next dependency — DISCHARGED by REAL-DATA-04 (see the row above).** The prediction below was recorded before the data existed and **its first branch is what happened**: D↔A succeeded (1656 inliers) and D↔B failed (3). Illumination is the driver and D-036 is superseded by D-040. **Frame identity is substantially weakened rather than conclusively refuted** — the argument joins two stages and rests on one succeeding edge per frame for A and D, and **REAL-DATA-05's replication returned UNRESOLVED**. The original wording is kept as written:
