...
**Addendum 2026-09-05 — XFeat (engine B4X).** Source (primary): https://github.com/verlab/accelerated_features (Potje et al., CVPR 2024), retrieved 2026-09-05: repository **Apache-2.0** (LICENSE file); the pretrained `xfeat.pt` is distributed inside that repository's tree and the README states no separate weight licence — recorded as *inherits the repository licence, not separately asserted by the authors*. The engine is loaded through `torch.hub` from a **pinned commit** (`e92685f57f8318b18725c5c8c0bd28c7fe188d9a`, 2025-01-15), which downloads and executes that commit's code at first use and caches it under `~/.cache/torch/hub`; this is stated in `siim.baselines.xfeat` and the README. The LighterGlue head is not used (a second weight file with a longer licence trail).
